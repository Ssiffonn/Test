...
<body class="p-3 mb-2 bg-dark text-white">
            ...
            <div class="border border-secondary rounded-3 d-flex flex-row justify-content-evenly text-center align-items-center p-3 bg-secondary" style="width: 50%;">
                ...
                <div class="">
                    <div class="d-flex flex-column justify-content-start text-start">
                        <div>
                            <input type="checkbox" name="Answers[]" value="1-5-1" class="form-check-input">
                            <label>A) FIFO</label>
                        </div>
                        <div>
                            <input type="checkbox" name="Answers[]" value="1-5-2" class="form-check-input">
                            <label>B) FILO</label>
                        </div>
                        <div>
                            <input type="checkbox" name="Answers[]" value="1-5-3" class="form-check-input">
                            <label>C) LIFO</label>
                        </div>
                        <div>
                            <input type="checkbox" name="Answers[]" value="1-5-4" class="form-check-input">
                            <label>D) LILO</label>
                        </div>
                    </div>
                    <div id="butts">
                        <input type="submit" value="Предыдущий вопрос" name="prev" class="rounded-pill btn btn-outline-light mt-3">
                        <input type="submit" value="Показать результат" name="next" class="rounded-pill btn btn-outline-light mt-3">
                    </div>
                </div>
            </div>
        </div>
    ...
    <?php
        ...
        if(isset($_POST['next']) && !empty($_POST['Answers'])){
            $_SESSION['Answers_5']=$_POST['Answers'];
            $_SESSION['Question_5'] = makeQuestionDTO();
            ?>
                <meta http-equiv="refresh" content="0; url=/showRes">
            <?php
        }
        if(isset($_POST['prev'])){
            ?>
